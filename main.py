...
class VoterCard(BoxLayout):
    ...
    def upd(self, *args):
        ...
        if hasattr(self, 'border'): self.border.pos, self.border.size = self.pos, self.size
...
class ElectionWarRoomApp(App):
    ...
    def build_sidebar(self):
        ...
        for t, f in btns:
            ...
            b.bind(on_release=f); self.sidebar.add_widget(b)
    ...
    def exp_slip(self, fmt, v):
        ...
        def fin(dt):
            ...
            else: self.msg("सफल", f"इमेज सुरक्षित फोल्डर में सेव हुई:\n{path}")
            self.p_slip.dismiss()
    ...
    def tool_density(self, *args):
        self.toggle_sb(); conn = sqlite3.connect(DB_PATH); c = conn.cursor()
        c.execute("SELECT house, COUNT(*) as c FROM voters GROUP BY house ORDER BY c DESC LIMIT 20")
        self.msg("सघन मकान (टॉप 20)", "\n".join([f"मकान नं. {x[0]} ➔ {x[1]} वोट" for x in c.fetchall()])); conn.close()
    ...
    def tool_margin(self, *args):
        self.toggle_sb(); conn = sqlite3.connect(DB_PATH); c = conn.cursor()
        c.execute("SELECT COUNT(*), SUM(voted) FROM voters")
        t, v = c.fetchone(); v = v or 0; tgt = int(t * 0.55); conn.close()
    ...
    def tool_excel(self, *args):
        ...
        conn = sqlite3.connect(DB_PATH)
        df = pd.read_sql_query("SELECT part, sr_no, epic, name, rel_name, house, age, mobile, tag, voted, is_deleted FROM voters ORDER BY name ASC", conn)
        conn.close()
