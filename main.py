...
def build_hinglish(text):
    if not text:
        return ""
    return "".join([CHAR_MAP.get(ch, VOWEL_MAP.get(ch, ch.lower())) for ch in unicodedata.normalize('NFC', text) if ch.isalnum() or ch in CHAR_MAP or ch in VOWEL_MAP])
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS voters (
        ...
        name TEXT, rel_type TEXT, rel_name TEXT, house TEXT, age INTEGER, gender TEXT, hinglish TEXT,
    ...
    for page in reader.pages[2:]:
        ...
        for m in pat.findall(unicodedata.normalize('NFC', raw)):
            ...
            c.execute('''INSERT OR REPLACE INTO voters 
                (ward, part, sr_no, epic, name, rel_type, rel_name, house, age, gender, hinglish, is_deleted, mobile, tag, notes, voted) 
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?, 
                ...
                ("12", str(part_no), int(sr), epic.strip(), c_nm, rel_t.strip(), clean_hindi(rel_n), 
                 clean_hindi(hs) or "-", int(age), "स्त्री" if "स्त्री" in gen else "पुरुष", build_hinglish(c_nm), is_del,
...
TAG_COLORS = {
    "सामान्य": (0.5, 0.5, 0.5, 1),
    "पक्का समर्थक": (0.1, 0.7, 0.2, 1),
    "विरोधी": (0.8, 0.1, 0.1, 1),
    "संदेहास्पद": (0.9, 0.5, 0.1, 1),
    "प्रवासी": (0.2, 0.5, 0.8, 1),
    "VIP": (0.8, 0.2, 0.6, 1)
}
    ...
    def __init__(self, v, action_cb, **kwargs):
        ...
        with self.canvas.before:
            if v['is_deleted']:
                ...
                Color(0.88, 0.98, 0.88, 1)
            else:
                Color(0.96, 0.97, 0.99, 1)
            self.bg = RoundedRectangle(pos=self.pos, size=self.size, radius=[8])
            if v['is_deleted']:
                Color(0.9, 0.1, 0.1, 1)
                self.border = RoundedRectangle(pos=self.pos, size=self.size, radius=[8], line_width=1.5)
        ...
        b_call = Button(text="📞", font_size='11sp', size_hint_x=0.2, background_color=(0.1, 0.6, 0.2, 1) if v['mobile'] else (0.6, 0.6, 0.6, 1))
        
        r3.add_widget(b_fam)
        r3.add_widget(b_edit)
        r3.add_widget(b_call)
        self.add_widget(r3)
        ...
        r4.add_widget(b_slip)
        r4.add_widget(b_vote)
        self.add_widget(r4)
...
class ElectionWarRoomApp(App):
    def build(self):
        ...
        # Scoped Storage Handling for Android 16
        if platform == 'android':
            from jnius import autoclass
            PythonActivity = autoclass('org.kivy.android.PythonActivity')
            Environment = autoclass('android.os.Environment')
            context = PythonActivity.mActivity
            ext_dir = context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS)
            self.exp_dir = ext_dir.getAbsolutePath() if ext_dir else context.getFilesDir().getAbsolutePath()
        else:
        ...
        self.lbl_top = Label(text="वॉर-रूम (ऑफ़लाइन)", bold=True, font_size='13sp')
        tb.add_widget(b_menu)
        tb.add_widget(self.lbl_top)
        ...
        s_box.add_widget(self.t_search)
        s_box.add_widget(b_clr)
        ...
        self.s_tag = Spinner(text="टैग", values=("टैग", "पक्का समर्थक", "विरोधी", "संदेहास्पद", "प्रवासी", "VIP"), size_hint_x=0.3)
        self.s_vote = Spinner(text="मतदान", values=("मतदान", "वोट बाकी", "वोट दिया"), size_hint_x=0.28)
        self.s_age = Spinner(text="फ़िल्टर", values=("फ़िल्टर", "18-21 (युवा)", "60+ (वरिष्ठ)", "80+ (वाहन)", "नंबर नहीं"), size_hint_x=0.22)
        
        for s in [self.s_part, self.s_tag, self.s_vote, self.s_age]:
            s.font_size = '11sp'
            s.bind(text=self.refresh)
            f1.add_widget(s)
    ...
    def update_rects(self, *args):
        self.sb_bg.pos = self.sidebar_container.pos
        self.sb_bg.size = self.sidebar_container.size
        self.tb_bg.pos = self.tb_bg.pos
        self.tb_bg.size = self.tb_bg.size
    ...
    def refresh(self, *args):
        ...
        if self.s_part.text != "भाग":
            q += " AND part = ?"
            p.append(self.s_part.text)
        if self.s_tag.text != "टैग":
            q += " AND tag = ?"
            p.append(self.s_tag.text)
        if self.s_vote.text == "वोट दिया":
            q += " AND voted=1"
        elif self.s_vote.text == "वोट बाकी":
            q += " AND voted=0"
        
        if self.s_age.text == "18-21 (युवा)":
            q += " AND age BETWEEN 18 AND 21"
        elif self.s_age.text == "60+ (वरिष्ठ)":
            q += " AND age >= 60"
        elif self.s_age.text == "80+ (वाहन)":
            q += " AND age >= 80"
        elif self.s_age.text == "नंबर नहीं":
            q += " AND mobile=''"

        st = self.t_search.text.strip()
        if st:
            q += " AND (name LIKE ? OR hinglish LIKE ? OR epic LIKE ? OR house LIKE ? OR mobile LIKE ?)"
            p.extend([f"%{st}%"] * 5)
        ...
        for r in rows:
            self.list_lyt.add_widget(VoterCard(dict(r), self.card_action))
        conn.close()
    ...
    def popup_slip(self, v):
        ...
        if os.path.exists(self.poster):
            self.slip_lyt.add_widget(Image(source=self.poster, size_hint_y=0.4, allow_stretch=True, keep_ratio=False))
        else:
            self.slip_lyt.add_widget(Label(text="[पोस्टर यहाँ दिखेगा]", color=(0.4, 0.4, 0.4, 1), size_hint_y=0.4))
    ...
    def exp_slip(self, fmt, v):
        png_path = os.path.join(self.exp_dir, f"Slip_{v['epic']}.png")
        self.slip_lyt.export_to_png(png_path)
        
        def fin(dt):
            if fmt == 'pdf':
                pdf_path = os.path.join(self.exp_dir, f"Slip_{v['epic']}.pdf")
                try:
                    pdf = FPDF(unit="mm", format="A4")
                    pdf.add_page()
                    pdf.image(png_path, x=15, y=15, w=180)
                    pdf.output(pdf_path)
                    if os.path.exists(png_path):
                        os.remove(png_path)
                    self.msg("सफल", f"PDF सुरक्षित फोल्डर में सेव हुई:\n{pdf_path}")
                except Exception as e:
                    self.msg("त्रुटि", f"PDF नहीं बन सकी: {str(e)}")
            else:
                self.msg("सफल", f"इमेज सुरक्षित फोल्डर में सेव हुई:\n{png_path}")
            self.p_slip.dismiss()
    ...
    def tool_pdf(self, *args):
        self.toggle_sb()
        b = BoxLayout(orientation='vertical', spacing=8, padding=10)
        ...
        def run(_):
            ...
            lbl.text = "प्रोसेसिंग..."
            def w():
                cnt = extract_pdf(fc.selection[0], sp.text)
                lbl.text = f"सफल! {cnt} वोटर लोड हुए।"
                self.refresh()
    ...
    def tool_density(self, *args):
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT house, COUNT(*) as cnt FROM voters GROUP BY house ORDER BY cnt DESC LIMIT 20")
        self.msg("सघन मकान (टॉप 20)", "\n".join([f"मकान नं. {x[0]} ➔ {x[1]} वोट" for x in c.fetchall()]))
        conn.close()
    ...
    def tool_dupe(self, *args):
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT name, rel_name, COUNT(*) as cnt FROM voters GROUP BY name, rel_name HAVING cnt>1")
        d = c.fetchall()
        conn.close()
    ...
    def tool_80plus(self, *args):
        self.toggle_sb()
        self.s_age.text = "80+ (वाहन)"
        self.refresh()

    def tool_missing_mob(self, *args):
        self.toggle_sb()
        self.s_age.text = "नंबर नहीं"
        self.refresh()

    def tool_evm(self, *args):
        self.toggle_sb()
        b = BoxLayout(orientation='vertical', spacing=6, padding=10)
        ...
        def sv(_):
            ...
            conn.close()
            p.dismiss()
            self.msg("सफल", "EVM डिटेल्स सेव हो गईं।")
        btn.bind(on_release=sv)
        p.open()
    ...
    def tool_incident(self, *args):
        ...
        b.add_widget(btn)
        p = Popup(title="घटना डायरी", content=b, size_hint=(0.85, 0.4))
        ...
        def sv(_):
            ...
            conn.close()
            p.dismiss()
            self.msg("सफल", "घटना रिकॉर्ड हो गई।")
        btn.bind(on_release=sv)
        p.open()
    ...
    def tool_excel(self, *args):
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT part, sr_no, epic, name, rel_name, house, age, mobile, tag, voted, is_deleted FROM voters ORDER BY name ASC")
        rows = c.fetchall()
        conn.close()
        
        wb = Workbook()
        ws = wb.active
        ws.title = "मतदाता सूची"
        ws.append(['भाग', 'क्रम', 'EPIC', 'नाम', 'संबंधी', 'मकान', 'आयु', 'मोबाइल', 'टैग', 'वोट दिया', 'विलोपित'])
        for r in rows:
            ws.append(list(r))
        wb.save(fp)
        self.msg("एक्सेल एक्सपोर्ट", f"A to Z सूची आपके सुरक्षित फोल्डर में सेव हो गई:\n{fp}")
    ...
    def tool_poster(self, *args):
        ...
        b.add_widget(btn)
        p = Popup(title="पोस्टर चुनें", content=b, size_hint=(0.9, 0.8))
        ...
        def st(_):
            if fc.selection:
                ...
                self.poster = "my_photo.png"
                p.dismiss()
                self.msg("सफल", "पर्ची के लिए आपका पोस्टर सेट हो गया है।")
        btn.bind(on_release=st)
        p.open()
    ...
    def msg(self, title, text):
        ...
        b.add_widget(btn)
        p = Popup(title=title, content=b, size_hint=(0.85, 0.6))
